  ...
  for(var i = 0; i < cart.length; i++){
    var total = cart[i].product.price * cart[i].product.sl;
      ...
      <tr style="color: white">
              <th style="padding: 40px 40px 0;">STT</th>
              <th style="padding: 40px 40px 0 0;">Image</th>            
              <th style="padding: 40px 40px 0 0;">name</th>
              <th style="padding: 40px 40px 0 0;">Số lượng</th>
              <th style="padding: 40px 40px 0 0;">price</th>
              <th style="padding: 40px 40px 0 0;">Thành tiền</th>
            </tr>
      <tr style="color: white">     
...
function getData(){
    var cartListJSON = localStorage.getItem("cart");

    // Nếu localStorage null (ko có ds) thì ko làm gì cả.
    if(!cartListJSON) return;

    // Chuyển lại chuỗi thành mảng đối tượng
    var cartListLocal = JSON.parse(cartListJSON);
    cart = mapData(cartListLocal);

    // in lại danh sách ra màn hình
    renderCart(cart); 
}
...
function mapData(dataFromLocal) {
  ...
  for (var i = 0; i < dataFromLocal.length; i++) {
    var oldCart = dataFromLocal[i];
    var newCart = new cartItem(

      oldCart.product.id,
      oldCart.product.img,
      oldCart.product.price,
      oldCart.roduct.name,
      oldCart.product.sl,
      oldCart.product.money,
